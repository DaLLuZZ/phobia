...
reg_proc_mmps(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	...
	if (lval != NULL) {
		...
		lval->f = rpm.f * pm.const_ld_Sm * (1000.f / 60.f);
	}
	else if (rval != NULL) {

		if (pm.const_ld_Sm > M_EPSILON) {

			rpm.f = rval->f / pm.const_ld_Sm * (60.f / 1000.f);
...
reg_proc_kmh(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	...
	if (lval != NULL) {
		...
		lval->f = rpm.f * pm.const_ld_Sm * (3.6f / 60.f);
	}
	else if (rval != NULL) {

		if (pm.const_ld_Sm > M_EPSILON) {

			rpm.f = rval->f / pm.const_ld_Sm * (60.f / 3.6f);
...
reg_proc_current_pc(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	if (lval != NULL) {
		...
		if (reg->link->f > 0.f) {
			...
			lval->f = reg->link->f * 100.f / pm.i_reverse;
		}
	}
	else if (rval != NULL) {
		...
		if (rval->f > 0.f) {
			...
			reg->link->f = rval->f * pm.i_reverse / 100.f;
		}
	}
}

static void
reg_proc_torque_pc(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	float			mQ;
	...
	if (lval != NULL) {
		...
		if (reg->link->f > 0.f) {
			...
			mQ = pm_torque_maximal(&pm, pm.i_maximal);
		}
		else {
			mQ = pm_torque_maximal(&pm, pm.i_reverse);
		}

		lval->f = reg->link->f * 100.f / mQ;
	}
	else if (rval != NULL) {
...
reg_proc_knob(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	if (pm.config_LU_DRIVE == PM_DRIVE_CURRENT) {
		...
		reg_proc_current_pc(reg, lval, rval);
	}
	else if (pm.config_LU_DRIVE == PM_DRIVE_TORQUE) {

		reg = &regfile[ID_PM_I_SETPOINT_TORQUE_PC];

		reg_proc_torque_pc(reg, lval, rval);
	}
...
reg_proc_lambda_nm(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	...
	if (lval != NULL) {
		...
		mQ = pm_torque_maximal(&pm, pm.i_maximal) / pm.i_maximal;
...
reg_proc_kg(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	...
	const float		lR = pm.const_ld_Sm / M_2_PI_F;
...
reg_proc_weakening(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	...
	if (lval != NULL) {
		...
		weak = pm.const_lambda - reg->link->f * pm.const_im_Ld;
		...
		if (		pm.const_im_Ld > M_EPSILON
			...
			reg->link->f = (pm.const_lambda - weak) / pm.const_im_Ld;
		}
		else {
			reg->link->f = 0.f;
		}
...
reg_proc_location_mm(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	if (lval != NULL) {
		...
		lval->f = reg->link->f * (pm.const_ld_Sm * 1000.f)
...
reg_proc_x_accel_mm(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	...
	if (lval != NULL) {
		...
		lval->f = rad.f * (pm.const_ld_Sm * 1000.f)
			/ (M_2_PI_F * (float) pm.const_Zp);
	}
	else if (rval != NULL) {
...
reg_proc_tlm_rate(const reg_t *reg, rval_t *lval, const rval_t *rval)
{
	if (lval != NULL) {
		...
		if (		rval->f <= hal.PWM_frequency
				&& rval->f >= 0.1f) {

			rate = (int) (hal.PWM_frequency / rval->f + 0.5f);
		}
...
const reg_t		regfile[] = {
	...
	REG_DEF(null,,,				"",	"%0i",	REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	...
	REG_DEF(ap.step_const_Sm,,,	"rad/step",	"%4f",	REG_CONFIG, NULL, NULL),
	REG_DEF(ap.step_const_Sm, _deg,, "deg/step",	"%4f",	0, &reg_proc_location_deg, NULL),
	REG_DEF(ap.step_const_Sm, _mm,,	"mm/step",	"%4f",	0, &reg_proc_location_mm, NULL),
#endif /* HW_HAVE_STEP_DIR_KNOB */
	...
	REG_DEF(ap.temp_gain_LP,,,		"",	"%2e",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(pm.self_RMSu,,,			"V",	"%4f",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.self_RMSt,,,			"",	"%0i",	REG_READ_ONLY, NULL, &reg_format_self_RMSt),
	REG_DEF(pm.self_DTu,,,			"V",	"%4f",	REG_READ_ONLY, NULL, NULL),
	...
	REG_DEF(pm.config_DCU_VOLTAGE,,,	"",	"%0i",	REG_CONFIG, NULL, &reg_format_enum),
	...
	REG_DEF(pm.config_CC_BRAKE_STOP,,,	"",	"%0i",	REG_CONFIG, NULL, &reg_format_enum),
	...
	REG_DEF(pm.vsi_A0,,,			"",	"%0i",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.vsi_B0,,,			"",	"%0i",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.vsi_C0,,,			"",	"%0i",	REG_READ_ONLY, NULL, NULL),

	REG_DEF(pm.dcu_deadband,,,		"ns",	"%1f",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.dcu_tol,,,			"A",	"%3f",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.dcu_DX,,,			"V",	"%3f",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.dcu_DY,,,			"V",	"%3f",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.dcu_X,,,			"V",	"%3f",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.dcu_Y,,,			"V",	"%3f",	REG_READ_ONLY, NULL, NULL),
	...
	REG_DEF(pm.kalman_rsu_D,,,		"A",	"%3f",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.kalman_rsu_Q,,,		"A",	"%3f",	REG_READ_ONLY, NULL, NULL),
	...
	REG_DEF(pm.hall_ST, 2_X, [2].X,"",	"%3f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.hall_ST, 2_Y, [2].Y,"",	"%3f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.hall_ST, 2, [2],	"deg",	"%2f",	REG_READ_ONLY, &reg_proc_fpos_nolock_deg, NULL),
	REG_DEF(pm.hall_ST, 3_X, [3].X,"",	"%3f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.hall_ST, 3_Y, [3].Y,"",	"%3f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.hall_ST, 3, [3],	"deg",	"%2f",	REG_READ_ONLY, &reg_proc_fpos_nolock_deg, NULL),
	REG_DEF(pm.hall_ST, 4_X, [4].X,"",	"%3f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.hall_ST, 4_Y, [4].Y,"",	"%3f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.hall_ST, 4, [4],	"deg",	"%2f",	REG_READ_ONLY, &reg_proc_fpos_nolock_deg, NULL),
	REG_DEF(pm.hall_ST, 5_X, [5].X,"",	"%3f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.hall_ST, 5_Y, [5].Y,"",	"%3f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.hall_ST, 5, [5],	"deg",	"%2f",	REG_READ_ONLY, &reg_proc_fpos_nolock_deg, NULL),
	REG_DEF(pm.hall_ST, 6_X, [6].X,"",	"%3f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.hall_ST, 6_Y, [6].Y,"",	"%3f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	...
	REG_DEF(pm.sincos_CONST, 0, [0],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 1, [1],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 2, [2],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 3, [3],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 4, [4],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 5, [5],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 6, [6],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 7, [7],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 8, [8],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 9, [9],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 10, [10],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 11, [11],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 12, [12],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 13, [13],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 14, [14],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),
	REG_DEF(pm.sincos_CONST, 15, [15],	"",	"%6f",	REG_CONFIG | REG_READ_ONLY | REG_HIDDEN, NULL, NULL),

	REG_DEF(pm.sincos_const_Zs,,,		"",	"%0i",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.sincos_const_Zq,,,		"",	"%0i",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.sincos_wS,,,		"rad/s",	"%2f",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.sincos_wS, _rpm,,		"rpm",	"%2f",	REG_READ_ONLY, &reg_proc_rpm, NULL),
	REG_DEF(pm.sincos_wS, _mmps,,		"mm/s",	"%2f",	REG_READ_ONLY, &reg_proc_mmps, NULL),
	REG_DEF(pm.sincos_gain_PF,,,		"",	"%2e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.sincos_gain_SF,,,		"",	"%2e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.sincos_gain_IF,,,		"%",	"%1f",	REG_CONFIG, &reg_proc_percent, NULL),
	...
	REG_DEF(pm.const_im_Ld,,,		"H",	"%4g",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.const_im_Lq,,,		"H",	"%4g",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.const_im_A,,,		"deg",	"%1f",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.const_im_Rz,,,		"Ohm",	"%4g",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.const_ld_Sm,,,		"mm",	"%3f",	REG_CONFIG, &reg_proc_mm, NULL),
	...
	REG_DEF(pm.watt_gain_WF,,,		"",	"%2e",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(pm.s_accel_forward,,,	"rad/s2",	"%1f",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.s_accel_forward, _rpm,,"rpm/s",	"%1f",	0, &reg_proc_rpm, NULL),
	REG_DEF(pm.s_accel_forward, _kmh,,"km/h/s",	"%1f",	0, &reg_proc_kmh, NULL),
	REG_DEF(pm.s_accel_reverse,,,	"rad/s2",	"%1f",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.s_accel_reverse, _rpm,,"rpm/s",	"%1f",	0, &reg_proc_rpm, NULL),
	REG_DEF(pm.s_accel_reverse, _kmh,,"km/h/s",	"%1f",	0, &reg_proc_kmh, NULL),
	...
	REG_DEF(pm.l_track_tol, _rpm,,	"rpm",		"%2f",	0, &reg_proc_rpm, NULL),
	REG_DEF(pm.l_track_tol, _kmh,,	"km/h",		"%1f",	0, &reg_proc_kmh, NULL),
	...
	REG_DEF(tlm.reg_ID, 10, [10],		"",	"%0i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(tlm.reg_ID, 11, [11],		"",	"%0i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(tlm.reg_ID, 12, [12],		"",	"%0i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(tlm.reg_ID, 13, [13],		"",	"%0i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(tlm.reg_ID, 14, [14],		"",	"%0i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(tlm.reg_ID, 15, [15],		"",	"%0i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(tlm.reg_ID, 16, [16],		"",	"%0i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(tlm.reg_ID, 17, [17],		"",	"%0i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(tlm.reg_ID, 18, [18],		"",	"%0i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(tlm.reg_ID, 19, [19],		"",	"%0i",	REG_CONFIG | REG_LINKED, NULL, NULL),
